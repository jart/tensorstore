...
class CcFlags(object):
  ...
  def add_if_supported(self, description='', copts=(), ldflags=()):
    ...
    if not self._check_flags(copts, ldflags):
...
class BuildExt(build_ext.build_ext):
  def build_extensions(self):
    ...
    if self.compiler.compiler_type == 'msvc':
      ...
      # TODO(jart): Old Ubuntu14 + Clang5 results in enable_if_t error.
      # if not cxxflags.add_if_supported(copts=['-std=c++14']):
      ...
      # folks who don't tune get tiny fast binaries that can't be distributed
      if not (os.environ.get('CFLAGS') or
              os.environ.get('CXXFLAGS')):
        cflags.add_if_supported(
            copts=[
                '-g0',  # order of magnitude binary size reduction
                '-O3',  # better at small than -Os for pybind11
                '-DNDEBUG',  # removes assert(), DCHECK(), etc.
            ],
            ldflags=['-g0'])
        cflags.add_if_supported(
            '-march=native (NOTE: THIS IS FAST BUT YOU NEED TO SET CFLAGS, '
            'CXXFLAGS, ETC. IF YOU INTEND TO DISTRIBUTE THIS BINARY)',
            copts=['-march=native'])
